...
When('preenche os campos de primeiro nome, último nome e ID', () => {
    ...
    cy.get('.oxd-grid-item > .oxd-input-group > :nth-child(2) > .oxd-input').should('be.visible')
        .and('be.enabled')
        .clear()
...
Then('o sistema deve exibir os dados do usuário na listagem', () => {
    cy.get('.oxd-table-card > .oxd-table-row').should('be.visible')
        .and('contain', cadastropim.cadastroValido.id)
        .and('contain', cadastropim.cadastroValido.primeiroNome)
        .and('contain', cadastropim.cadastroValido.ultimoNome);
});
